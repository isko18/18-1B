...
async def register_business_user(user_id, username, referrer_id=None):
    if referrer_id and referrer_id != str(user_id):
        referrer_user = await sync_to_async(UserBusiness.objects.filter(user_id=referrer_id).first)()
        if referrer_user:
            ...
            return referrer_id, f"По вашей реферальной ссылке зарегистрировался новый пользователь: {username}"
    await sync_to_async(UserBusiness.objects.create)(
        user_id=user_id,
        username=username
    )
...
async def register_client_user(user_id, username, referrer_id=None):
    if referrer_id:
        referrer_user = await sync_to_async(UserCklient.objects.filter(user_id=referrer_id).first)()
        if referrer_user:
...
async def start(message: types.Message):
    ...
    if user_business is None and user_client is None:
        referrer_id_business, referrer_message_business = await register_business_user(user_id, username, referrer_id)
        referrer_id_client, referrer_message_client = await register_client_user(user_id, username, referrer_id)
        
        if referrer_message_business:
            await bot.send_message(referrer_id_business, referrer_message_business)
            await message.answer(f"Вы зарегистрировались по реферальной ссылке пользователя {referrer_id_business}")
        
        if referrer_message_client:
            await bot.send_message(referrer_id_client, referrer_message_client)
            await message.answer(f"Вы зарегистрировались по реферальной ссылке пользователя {referrer_id_client}")
...
async def register_hotel_handler(query: types.CallbackQuery, state: FSMContext):
    ...
    invited_user_count = await sync_to_async(UserBusiness.objects.filter(referrer_id=user_id).count)()

    if invited_user_count < 1:
        invite_link = f"https://t.me/redis_bot?start={user_id}"
        await query.message.edit_text(
            f"Пожалуйста, пригласите хотя бы одного пользователя через вашу реферальную ссылку перед завершением. Ваша реферальная ссылка: {invite_link}",
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton('Отправить', switch_inline_query=invite_link),
                InlineKeyboardButton('Назад', callback_data='back_to_photos'),
                InlineKeyboardButton('Разместить отель', callback_data='register_hotel')
            )
        )
        await query.answer(f"Пожалуйста, пригласите одного пользователя и попробуйте снова. {invited_user_count}/1", show_alert=True)
        return
    ...
    async with state.proxy() as data:
        ...
        photos = data.get('photos')
        referrer_id = user_business.referrer_id
...
async def invite_users_handler(query: types.CallbackQuery):
    user_id = query.from_user.id
    invite_link = f"https://t.me/redis_bot?start={user_id}"
    await query.message.edit_text(
        f"Пожалуйста, пригласите хотя бы одного пользователя через вашу реферальную ссылку. Ваша реферальная ссылка: {invite_link}",
        reply_markup=InlineKeyboardMarkup().add(
            InlineKeyboardButton('Отправить', switch_inline_query=invite_link),
            InlineKeyboardButton('Назад', callback_data='back_to_photos'),
            InlineKeyboardButton('Разместить отель', callback_data='register_hotel')
...
def register_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(invite_users_handler, text='invite_users', state='*')